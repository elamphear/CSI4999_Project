...
inputWordValues = []
exclusionWordList = []
key = "2bdc6c4e1c82478cb5adb0ad0c113314"
endpoint = "https://webcat-sentiment.cognitiveservices.azure.com/"
...
def sentiment_analysis(client, document):

    inputSentence = ['"' + document + '"']
    response = client.analyze_sentiment(inputs=inputSentence)[0]

    print("\nSentence Sentiment: {}".format(response.sentiment))
...
def parse():
    ...
    for sentence in articleData:
        ...
        sentimentBias = sentiment_analysis(client, sentence['sentence'])
        ...
        for inputWord in inputWordList:
            ...
            if match:
                ...
                if sentimentBias == 'positive':
                    value *= 2
				
                elif sentimentBias == 'negative':
                    value *= 2
				
                rankingValue += value

                print("Matched Keyword : " + inputWord + " : " + str(value))
    ...
    if (rankingPercent <= 150 and rankingPercent > 75):
    ...
    elif (rankingPercent < -75 and rankingPercent >= -150):
        rankingName = 'FarLeft'
    ...
    print(rankingName)
    print(json.dumps(topWordList, indent = 4, sort_keys=True))
